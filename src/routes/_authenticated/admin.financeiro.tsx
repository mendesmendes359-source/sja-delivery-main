...
function FinancePage() {
  ...
  const delExp = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["expenses"] }); qc.invalidateQueries({ queryKey: ["fin"] }); },
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-end gap-2">
          <label className="text-xs">De<input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="mt-1 block rounded border bg-background px-2 py-1 text-sm" /></label>
          <label className="text-xs">Até<input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="mt-1 block rounded border bg-background px-2 py-1 text-sm" /></label>
        </div>
      </div>
      ...
      {f && (
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Stat label="Lucro" value={formatMoney(f.profit_cents)} tone={f.profit_cents >= 0 ? "brand" : "destructive"} />
        </div>
      )}
        ...
        <div className="rounded-xl border bg-card p-5">
          ...
          <table className="mt-3 w-full text-sm">
            <thead className="text-left text-xs uppercase text-muted-foreground">
              <tr><th className="py-2">Data</th><th>Categoria</th><th>Descrição</th><th className="text-right">Valor</th><th></th></tr>
            </thead>
            <tbody className="divide-y">
              {(expenses.data ?? []).map((e) => (
                <tr key={e.id}>
                  ...
                  <td className="text-right"><button onClick={() => delExp.mutate(e.id)} className="p-1 text-brand"><Trash2 className="h-4 w-4" /></button></td>
                </tr>
              ))}
              {(expenses.data ?? []).length === 0 && <tr><td colSpan={5} className="py-6 text-center text-muted-foreground">Sem despesas no período</td></tr>}
            </tbody>
          </table>
        </div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "brand" | "destructive" }) {
  const color = tone === "brand" ? "text-brand" : tone === "destructive" ? "text-brand" : "text-navy";
  return (
    <div className="rounded-xl border bg-card p-5">
