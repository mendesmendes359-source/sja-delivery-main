...
const deliveryQO = queryOptions({
  ...
  queryFn: async () => {
    const { data: orders, error } = await supabase
      .from("orders")
      .select("id, order_number, customer_name, customer_phone, address, status, total_cents, created_at")
      .eq("order_type", "entrega")
      .in("status", ["aceite", "em_preparacao", "saiu_entrega", "entregue"])
      .order("created_at", { ascending: false })
      .limit(50);
    if (error) throw error;
    const { data: dels } = await supabase.from("deliveries").select("id, order_id, courier_name, status, dispatched_at, delivered_at");
...
export const Route = createFileRoute("/_authenticated/admin/entregas")({
  loader: ({ context }) => context.queryClient.ensureQueryData(deliveryQO),
...
function DeliveriesAdmin() {
  ...
  const assignMut = useMutation({
    mutationFn: async (v: { order_id: string; courier_name: string }) => {
      ...
      if (existing) {
        const { error } = await supabase.from("deliveries").update({ courier_name: v.courier_name }).eq("id", existing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("deliveries").insert({ order_id: v.order_id, courier_name: v.courier_name, status: "atribuido" });
        if (error) throw error;
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin", "deliveries"] }); toast.success("Estafeta atribuído"); },
  ...
  const markMut = useMutation({
    mutationFn: async (v: { order_id: string; kind: "dispatch" | "delivered" }) => {
      ...
      if (v.kind === "dispatch") {
        await supabase.from("deliveries").upsert({ order_id: v.order_id, status: "em_transito", dispatched_at: now }, { onConflict: "order_id" });
        await supabase.from("orders").update({ status: "saiu_entrega" }).eq("id", v.order_id);
      } else {
        await supabase.from("deliveries").upsert({ order_id: v.order_id, status: "entregue", delivered_at: now }, { onConflict: "order_id" });
            ...
            <div key={o.id} className="rounded-xl border bg-card p-5">
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="mt-1 inline-block rounded-full bg-accent px-2 py-0.5 text-xs text-brand font-semibold">{o.status}</div>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                />
                <button
                  onClick={() => assignMut.mutate({ order_id: o.id, courier_name: couriers[o.id] ?? d?.courier_name ?? "" })}
                  className="rounded-md border px-3 py-1.5 text-sm hover:bg-muted"
                >
                  Atribuir
                </button>
                {o.status !== "saiu_entrega" && o.status !== "entregue" && (
                  <button onClick={() => markMut.mutate({ order_id: o.id, kind: "dispatch" })} className="rounded-md bg-navy px-3 py-1.5 text-sm text-navy-foreground">
                    Saiu p/ entrega
                  </button>
                )}
                {o.status !== "entregue" && (
                  <button onClick={() => markMut.mutate({ order_id: o.id, kind: "delivered" })} className="rounded-md bg-brand px-3 py-1.5 text-sm text-brand-foreground">
                    Entregue
                  </button>
                )}
              </div>
